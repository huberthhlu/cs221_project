...
class PttWebCrawler(object):
    ...
    def parse(link, article_id, board, timeout=3):
        ...
        # return None
        ####################
        if u'公告' in title or u'新聞' or u'通報' in title: 
            return None
        ####################
    ...
    def extractSentece(self):
        ...
        with open(filename, 'r') as csvfile: 
            ...
            for row in csvreader:
                ...
                sentences = cut_sent(corpus)
                
                for i in range(len(sentences)):
                    sentences[i] = Filter_Text().filtet_text(sentences[i]) # emoji filter!!!!
                    sentences[i] = removeRepeatedPunc(sentences[i])
                    if len(re.sub("^[0-9]*$", '', sentences[i])) <= 1 or ':/./' in sentences[i] or '/../' in sentences[i]: continue  # exclud the string only contains numbers
                    if sentences[i][:2] == ' :':
